...
public class CognitoServiceImpl implements CognitoService {
	...
	public String associateSoftwareTokenSessionId = null;
				...
				if (initiateAuthResponse != null && initiateAuthResponse instanceof NullNode == false) {
					...
					if (initiateAuthchallengeName != null && initiateAuthchallengeName.trim().equals("MFA_SETUP")) {
						...
						if (associateSoftwareTokenResponse != null
							...
							associateSoftwareTokenSessionId = associateSoftwareTokenResponse.get("Session").asText();
							if (secretCode != null ) {
								copyCodeToPropertYFile(secretCode,"db.secretCode");
							}
						}
					} else {
	...
	private void copyCodeToPropertYFile(String secretCode,String key) {

		try (OutputStream output = new FileOutputStream("application.properties")) {
			Properties prop = new Properties();
			
			prop.setProperty(key, secretCode);
							...
							if (authResultNode != null) {
								...
								if (refreshTokenNode != null) {
									...
									if (refreshToken != null) {
										copyCodeToPropertYFile(refreshToken,"db.refreshToken");
									}
								}
							}
						}
	...
	public String downloadCaseData(CognitoVO vo) {
		String idToken = vo.getIdToken();
		String res=null;
		try {
			// if (idToken == null || idToken.trim().equals(""))
			...
			res=sendGetCaseDataRequest(idToken);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return res;
	...
	public Object submitmfs(CognitoVO vo) {
		...
		String mfaToken = vo.getMfaToken();
		try {
			String VerifySoftwareTokenRespStr = sendPostVerifySoftwareToken(mfaToken, associateSoftwareTokenSessionId);
			...
			if (verifySoftwareTokenSoftwareResponse != null
				...
				if(StringUtils.isEmpty(status)) {
					vo.setMessage(	verifySoftwareTokenSoftwareResponse.get("message").asText());
				}else {
					vo.setMessage(status);
				}
	...
	public String getscCode() {
		...
		try (InputStream input = new FileInputStream("application.properties")) {
            ...
            prop.load(input);
            scCode=  prop.getProperty("db.secretCode");
	...
	public String getRefreshCode() {
		...
		try (InputStream input = new FileInputStream("application.properties")) {
            Properties prop = new Properties();
            prop.load(input);
            refreshCode=  prop.getProperty("db.refreshToken");
        } catch (IOException ex) {
            ex.printStackTrace();
        }
		return refreshCode;
	...
	public String getdocumentData(CognitoVO vo) {
		return downloadCaseData(vo);
	}

	@Override
	public Object getUserRole() {
		CognitoVO vo = new CognitoVO();
		vo.setUsername(userName1);
